...
class S3Facade(AWSBaseFacade):
        ...
        try:
            ...
            await get_and_set_concurrently(
                ...
                self._get_and_set_s3_bucket_logging,
                 ...
                 self._get_and_set_s3_custom_policy_action,
                 self._get_and_set_s3_custom_policy_principal
                 ],
                buckets)
    ...
    async def _get_and_set_s3_custom_policy_action(self, bucket: {}):
        client = AWSFacadeUtils.get_client('s3', self.session, bucket['region'])
        try:
            ...
            for c in bucket['policy']['Statement']:
                if c['Action'] != "*":
                    bucket['CheckActionPolicy'] = "True"
                else:
                    bucket['CheckActionPolicy'] = "False"
            # for policy in bucket['policy']['statement'][0]['Action']:
        except ClientError as e:
    ...
    async def _get_and_set_s3_custom_policy_principal(self, bucket: {}):
        client = AWSFacadeUtils.get_client('s3', self.session, bucket['region'])
        try:
            bucket_policy = await run_concurrently(lambda: client.get_bucket_policy(Bucket=bucket['Name']))
            bucket['policy'] = json.loads(bucket_policy['Policy'])
            for c in bucket['policy']['Statement']:
                if c['Principal'] != "*":
                    bucket['CheckPrincipalPolicy'] = "True"
                else:
                    bucket['CheckPrincipalPolicy'] = "False"
            # for policy in bucket['policy']['statement'][0]['Action']:
        except ClientError as e:
            if e.response['Error']['Code'] != 'NoSuchBucketPolicy':
                if 'NoSuchBucket' in str(e) or 'InvalidToken' in str(e):
